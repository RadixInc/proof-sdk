/**
 * Anchor resolution, copied verbatim from server/anchor-resolver.ts so the
 * agent ops port matches upstream addressing behavior exactly (issue #10);
 * the server/ tree is deleted by the fork-finalization slice. Only the
 * env-flag helpers were replaced with fixed defaults (see below).
 */
...
/*
 * Upstream toggled these via env; the Workers port fixes them at upstream's
 * hosted defaults (anchor v2 on, fail-closed duplicate handling on,
 * authored-span remap on, structural cleanup off — cleanup belongs to the
 * deleted repair machinery).
 */
